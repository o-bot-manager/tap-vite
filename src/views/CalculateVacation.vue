...
<template>
  <div class="container">
    ...
    <div v-if="result">
      ...
      <p>Итого начисленные отпускные: {{ result.vacationPayment }}</p>
      <p>Удержаны отчисления на социальное страхование (начисленная заработной платы * 10%): {{ result.socialTax }}</p>
      <p>Удержан подоходный налог (начисленная заработная плата минус отчисления на соцю страхование минус 650 сом) * 10%: {{ result.incomeTax }}</p>
      <p><b>Отпускные к выплате: {{ result.vacationPayAfterTax }}</b></p>
    </div>
  </div>
</template>
